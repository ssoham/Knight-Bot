...
public class hi extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase(Info.second_prefix + "add")) {
        ...
        } else if (args[0].equalsIgnoreCase(Info.second_prefix + "rem") || args[0].equalsIgnoreCase(Info.second_prefix + "remove")) {
            if (event.getMember().hasPermission(Permission.MANAGE_ROLES)) {
                Role role = getRole(args, event);
                if (event.getGuild().getSelfMember().canInteract(role)) {
                    Member member = event.getMessage().getMentionedMembers().get(0);
...
//                    message.addReaction("");
                    ...
                    EmbedBuilder e = new EmbedBuilder();
                    e.setDescription(role.getAsMention() + " was removed from " + member.getAsMention());
                    e.setColor(Color.yellow);
                    event.getChannel().sendMessage(e.build()).queue();
                }
                else {
                    EmbedBuilder e = new EmbedBuilder();
                    e.setColor(Color.yellow);
                    e.setDescription("I cannot remove a role higher than my own!");
                    event.getChannel().sendMessage(e.build()).queue();
                }
            }
            else {
